...
Public Function Build_CUS_031_SensorAngleBlock(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-031"
    partName = "Sensor angle block 10x8x2 cm"
    fileName = "CUS-031_SensorAngleBlock_100x80x20.SLDPRT"
    lengthMm = 100#
    widthMm = 80#
    heightMm = 20#
    quantity = 1#
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_031_SensorAngleBlock = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_031_SensorAngleBlock = ""
...
Fail:
    AppendLog "Build_CUS_031_SensorAngleBlock failed: " & Err.Description
