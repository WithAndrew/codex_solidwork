...
Public Function Build_CUS_020_SlideTopBlock(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-020"
    partName = "Slide top block 14.5x8x2 cm"
    fileName = "CUS-020_SlideTopBlock_145x80x20.SLDPRT"
    lengthMm = 145#
    widthMm = 80#
    heightMm = 20#
    quantity = 1#
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_020_SlideTopBlock = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_020_SlideTopBlock = ""
...
Fail:
    AppendLog "Build_CUS_020_SlideTopBlock failed: " & Err.Description
