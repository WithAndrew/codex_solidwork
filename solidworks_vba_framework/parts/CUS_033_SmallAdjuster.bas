...
Public Function Build_CUS_033_SmallAdjuster(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-033"
    partName = "Small adjuster 8x6x2 cm"
    fileName = "CUS-033_SmallAdjuster_80x60x20.SLDPRT"
    lengthMm = 80#
    widthMm = 60#
    heightMm = 20#
    quantity = 1#
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_033_SmallAdjuster = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_033_SmallAdjuster = ""
...
Fail:
    AppendLog "Build_CUS_033_SmallAdjuster failed: " & Err.Description
