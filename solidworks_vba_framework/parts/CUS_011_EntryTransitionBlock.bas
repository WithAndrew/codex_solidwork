...
Public Function Build_CUS_011_EntryTransitionBlock(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-011"
    partName = "Entry transition block 10x7x2 cm"
    fileName = "CUS-011_EntryTransitionBlock_100x70x20.SLDPRT"
    lengthMm = 100#
    widthMm = 70#
    heightMm = 20#
    quantity = 1#
    materialName = "TO_BE_CONFIRMED"
    photoReference = "13,14"
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_011_EntryTransitionBlock = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_011_EntryTransitionBlock = ""
...
Fail:
    AppendLog "Build_CUS_011_EntryTransitionBlock failed: " & Err.Description
