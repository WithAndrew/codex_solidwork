...
Public Function Build_CUS_006_ClampBlock(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-006"
    partName = "Clamp block 10x7x2 cm"
    fileName = "CUS-006_ClampBlock_100x70x20.SLDPRT"
    lengthMm = 100#
    widthMm = 70#
    heightMm = 20#
    quantity = 2#
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_006_ClampBlock = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_006_ClampBlock = ""
...
Fail:
    AppendLog "Build_CUS_006_ClampBlock failed: " & Err.Description
