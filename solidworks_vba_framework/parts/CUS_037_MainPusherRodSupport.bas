...
Public Function Build_CUS_037_MainPusherRodSupport(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-037"
    partName = "Main pusher rod support, 50 stroke mark to verify"
    fileName = "CUS-037_MainPusherRodSupport_100x70x50.SLDPRT"
    lengthMm = 100#
    widthMm = 70#
    heightMm = 50#
    quantity = 1#
    materialName = "TO_BE_CONFIRMED"
    photoReference = "51,52"
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_037_MainPusherRodSupport = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_037_MainPusherRodSupport = ""
...
Fail:
    AppendLog "Build_CUS_037_MainPusherRodSupport failed: " & Err.Description
