...
Public Function Build_CUS_016_SmallClampPlate(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-016"
    partName = "Small clamp plate 5x2.5x1 cm"
    fileName = "CUS-016_SmallClampPlate_50x25x10.SLDPRT"
    lengthMm = 50#
    widthMm = 25#
    heightMm = 10#
    quantity = 2#
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_016_SmallClampPlate = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_016_SmallClampPlate = ""
...
Fail:
    AppendLog "Build_CUS_016_SmallClampPlate failed: " & Err.Description
