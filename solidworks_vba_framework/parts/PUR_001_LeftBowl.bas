...
Public Function Build_PUR_001_LeftBowl(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "PUR-001"
    partName = "Left bowl feeder simplified envelope"
    fileName = "PUR-001_LeftBowl.SLDPRT"
    lengthMm = 480#
    widthMm = 480#
    heightMm = 280#
    quantity = 1#
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_PUR_001_LeftBowl = filePath
        AppendLog fileName & " saved"
    Else
        Build_PUR_001_LeftBowl = ""
...
Fail:
    AppendLog "Build_PUR_001_LeftBowl failed: " & Err.Description
