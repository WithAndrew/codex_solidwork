...
Public Function Build_CUS_036_MainPusherStrokeRail(ByVal swApp As Object, ByVal outputDir As String) As String
    On Error GoTo Fail
    ...
    partNo = "CUS-036"
    partName = "Main pusher stroke rail, 7.5 and 5 cm marks"
    fileName = "CUS-036_MainPusherStrokeRail_300x60x20.SLDPRT"
    lengthMm = 300#
    widthMm = 60#
    heightMm = 20#
    quantity = 1#
    materialName = "TO_BE_CONFIRMED"
    photoReference = "48,49,50"
    ...
    If SaveModel(swApp, model, filePath) Then
        Build_CUS_036_MainPusherStrokeRail = filePath
        AppendLog fileName & " saved"
    Else
        Build_CUS_036_MainPusherStrokeRail = ""
...
Fail:
    AppendLog "Build_CUS_036_MainPusherStrokeRail failed: " & Err.Description
